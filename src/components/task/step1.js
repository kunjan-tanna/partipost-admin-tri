...
function Stepone(props) {
  console.log("gg,", props.stepOne);
  // const [stepOne, setStepOneData] = useState(
  //   JSON.parse(localStorage.getItem("step1"))
  // );
  // console.log("DDDD", stepOne);
  ...
  const handleReqEdit = async (item) => {
    ...
    let res = await apiCall("POST", "acceptOrRejectRequest", reqData);
  ...
  const stepOneData =
    props.campaign.length > 0
      ? props.campaign.map((item, index) => {
          return stepOne.length > 0 ? (
            stepOne.map((task, index) => {
  ...
  return (
    <div>
      <div className="postItem">{stepOneData}</div>
